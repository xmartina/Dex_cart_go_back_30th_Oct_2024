...
<label>
{!! Form::radio('payment_method', 'bitcoin', false, ['class' => 'i-check-blue']) !!} Crypto Currency
</label>
</div>
  @php
    ...
    if ((isset($contain_digital_carts) && $contain_digital_carts) || $cart->is_digital) {
        $paymentMethods = $paymentMethods->reject(function ($paymentMethod) {
            return in_array($paymentMethod->code, ['cod', 'wire', 'crypto']);
        });
    }
  @endphp
...
<div class="form-group mb-5">
  <div class="checkbox">
    <label>
      {!! Form::checkbox('agree', null, null, ['class' => 'i-check-blue', 'required']) !!}
      {!! trans('theme.input_label.i_agree_with_terms', ['url' => route('page.open', \App\Models\Page::PAGE_TNC_FOR_CUSTOMER)]) !!}
    </label>
  </div>
